...
# Filter to keep only rows where 'sourceName' is "hashicorp/aws" and 'operator' is not "_"
df = df[(df['sourceName'] == 'hashicorp/aws) & (df['operator'] != '_')]
...
plt.title('Monthly Distribution of Technical Lag for hashicorp/aws)
plt.grid(True)
plt.tight_layout()
plt.show()
